...
export default class UserServices {
  ...
  getUserById = async (id: string): Promise<User> => {
    const user = await this._user.findOne({ where: { id: id } });
    if (!user) throw new NotFoundError(`Usuário de ID ${id} não encontrad!`);
  ...
  uploadImage = async (userId: string, imageUrl: string) => {
    await this._user.insert({ profile_picture: imageUrl });
  };
}
