...
export default class UserController {
  ...
  getUsersByName = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res
        .status(200)
        .json({ status: "success", total: users.length, users: users });
    } catch (error) {
      next(error);
    }
  };
  ...
  uploadUserImage = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!file) throw new BadRequestError("Arquivo faltando");
      const url = await this.userHelper.uploadToPostImages(file.path);
      await this.userServices.uploadImage(user_id.toString(), url);
